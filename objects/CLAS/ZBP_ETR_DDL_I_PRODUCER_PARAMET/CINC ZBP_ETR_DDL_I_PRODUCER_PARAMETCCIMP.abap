CLASS lhc_zetr_ddl_i_producer_parame DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zetr_ddl_i_producer_parameters RESULT result.

ENDCLASS.

CLASS lhc_zetr_ddl_i_producer_parame IMPLEMENTATION.
...
CLASS lhc_zetr_ddl_i_producer_serial DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zetr_ddl_i_producer_serials RESULT result.

    METHODS createNumbers FOR MODIFY
      IMPORTING keys FOR ACTION zetr_ddl_i_producer_serials~createNumbers RESULT result.

    METHODS checkSerials FOR VALIDATE ON SAVE
      IMPORTING keys FOR zetr_ddl_i_producer_serials~checkSerials.

ENDCLASS.

CLASS lhc_zetr_ddl_i_producer_serial IMPLEMENTATION.

  METHOD get_instance_authorizations.
  ENDMETHOD.

  METHOD createNumbers.
    DATA: lt_numbers      TYPE TABLE OF zetr_ddl_i_producer_numstat,
    ...
    IF sy-subrc <> 0 OR ls_key-%param-FiscalYear IS INITIAL.
      APPEND VALUE #( %msg = new_message( id       = 'ZETR_COMMON'
                                          number   = '217'
                                          severity = if_abap_behv_message=>severity-error ) ) TO reported-zetr_ddl_i_producer_parameters.
      RETURN.
    ENDIF.

    READ ENTITIES OF zetr_ddl_i_producer_parameters IN LOCAL MODE
      ENTITY zetr_ddl_i_producer_serials
      ALL FIELDS WITH CORRESPONDING #( keys )
      RESULT DATA(lt_serials).
    CHECK sy-subrc = 0.

    READ ENTITIES OF zetr_ddl_i_producer_parameters IN LOCAL MODE
      ENTITY zetr_ddl_i_producer_parameters
      ALL FIELDS WITH CORRESPONDING #( keys )
      ...
      CASE ls_parameter-InternalNumbering.
        WHEN abap_false.
          APPEND VALUE #( %msg = new_message( id       = 'ZETR_COMMON'
                                              ...
                                              v1 = ls_parameter-CompanyCode ) ) TO reported-zetr_ddl_i_producer_parameters.
        WHEN abap_true.
          ...
          IF lt_numbers IS NOT INITIAL.
            MODIFY ENTITIES OF zetr_ddl_i_producer_parameters IN LOCAL MODE
              ENTITY zetr_ddl_i_producer_serials
          ...
          APPEND VALUE #( %msg = new_message( id       = 'ZETR_COMMON'
                                              ...
                                              v2 = ls_key-%param-FiscalYear ) ) TO reported-zetr_ddl_i_producer_parameters.
      ENDCASE.
    ENDLOOP.
